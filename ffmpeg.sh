...
ffmpeg -f x11grab -s $(xwininfo -id $1| awk '/geometry/ {print $2}' | awk -F '[+,-]' '{print $1}') -r 25 -i :0.0+$(xwininfo -id $1 | awk '/Absolute upper-left X/ {print $4}'),$(xwininfo -id $1 | awk '/Absolute upper-left Y/ {print $4}') -vcodec libx264 -tune zerolatency -f rtp $2 
